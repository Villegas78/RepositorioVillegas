...
if(isset($_GET['aksi']) == 'delete'){
	...
	$cek = mysqli_query($connect, "SELECT * FROM materias WHERE nombre='$nik'");
	if(mysqli_num_rows($cek) == 0){
		echo '<div class="alert alert-info alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button> No se encontraron datos.</div>';
	}else{
		$delete = mysqli_query($connect, "DELETE FROM materias WHERE nombre='$nik'");
		if($delete){
...
// Encabezado de la tabla
	...
	echo "<tr>";
	echo "<th>Clave de la Materia</th>";
	echo "<th>Nombre</th>";
	echo "</tr>";
...
while ($columna = mysqli_fetch_array($resultado)){
		echo "<tr>";
		echo "<td>" . $columna['Clave_Materia'] . "</td><td>" . $columna['Nombre'] . "</td>";
		echo "</tr>";
}
